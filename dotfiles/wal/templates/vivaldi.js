...
colors.AccentBg = '{color2.rgb}'.split(',').concat(1.0);
colors.AccentBgAlpha = '{color2.rgb}'.split(',').concat(0.4);
colors.AccentBgAlphaHeavy = '{color2.rgb}'.split(',').concat(0.25);
colors.AccentBgDark = '{color2.rgb}'.split(',').concat(1.0);
colors.AccentBgDarker = '{color2.rgb}'.split(',').concat(1.0);
colors.AccentBgFaded = '{color10.rgb}'.split(',').concat(1.0);
colors.AccentBgFadedMore = '{color10.rgb}'.split(',').concat(1.0);
colors.AccentBgFadedMost = '{color10.rgb}'.split(',').concat(1.0);
...
//   document.getElementsByClassName("newtab")[0].style.display = 'none';
  ...
  // override css vars
  for (c in colors)
    app.style.setProperty('--color' + c, 'rgb(' + colors[c].join(',') + ')', "important");

  let browser = document.getElementById('browser');
  browser.style.setProperty("background-color", "rgb(" + colors.Bg.join(',') + ")", "important")
  // override css vars
  for (c in colors)
    browser.style.setProperty('--color' + c, 'rgb(' + colors[c].join(',') + ')', "important");
}};
